...
module.exports = {
  /**
   * @param {data, extract} object and array of int IDs of the values to extract, array can be of any length
  ...
  extractWaterValues: async (data) => {
    ...
    for (let i = 0; i < data.value.timeSeries.length; i++) {
      ...
      if (!(processedData.hasOwnProperty(
        data.value.timeSeries[i].sourceInfo.siteName)))
        {
          if (data.value.timeSeries[i].variable.variableCode[0].value==00065){
            //take the most recent value (for now)
            //because there are varying amounts of objects in the values array for each location
            gageHeight = data.value.timeSeries[i].values[0].value[0].value;
            gageHeightTime = data.value.timeSeries[i].values[0].value[0].dateTime;
            waterTemp = null;
            waterTempTime = null;
          }
          else {
            //since we only query for gageHeight and waterTemp(for now)
            waterTemp = data.value.timeSeries[i].values[0].value[0].value;
            waterTempTime = data.value.timeSeries[i].values[0].value[0].dateTime;
            gageHeight = null;
            gageHeightTime = null;
          }
          processedData[data.value.timeSeries[i].sourceInfo.siteName]={gageHeight, gageHeightTime, waterTemp, waterTempTime};
        }
      else 
        {
          ...
          if (processedData[data.value.timeSeries[i].sourceInfo.siteName].gageHeight===null){
            gageHeight = data.value.timeSeries[i].values[0].value[0].value;
            gageHeightTime = data.value.timeSeries[i].values[0].value[0].dateTime;
            waterTemp = processedData[data.value.timeSeries[i].sourceInfo.siteName].waterTemp;
            waterTempTime = processedData[data.value.timeSeries[i].sourceInfo.siteName].waterTempTime;
          }
          else {
            waterTemp = data.value.timeSeries[i].values[0].value[0].value;
            waterTempTime = data.value.timeSeries[i].values[0].value[0].dateTime;
            gageHeight = processedData[data.value.timeSeries[i].sourceInfo.siteName].gageHeight;
            gageHeightTime = processedData[data.value.timeSeries[i].sourceInfo.siteName].gageHeightTime;
          }
          processedData[data.value.timeSeries[i].sourceInfo.siteName]={gageHeight, gageHeightTime, waterTemp, waterTempTime};
        }
    }
